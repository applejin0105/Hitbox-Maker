...
public class AddHitbox : MonoBehaviour
{
    ...
    private static string _groundCheck = "GroundChecker";
    ...
    private static void Init()
    {
        ...
        List<string> tags = new List<string>() { "CharacterSkeleton", "Hitbox_Head", "Hitbox_Torso", "Hitbox_Arm", "Hitbox_Leg", "GroundChecker"};
        ...
        GameObject playerCapsule = FindDeepChild(_target, "PlayerCapsule");
        GameObject groundCheckerL = FindDeepChild(_target, "GroundChecker_L");
        GameObject groundCheckerR = FindDeepChild(_target, "GroundChecker_R");
        
        EndSetting(_target, playerCapsule, groundCheckerL, groundCheckerR);
    }
    ...
    private static GameObject FindDeepChild(GameObject node, string targetName)
    {
        ...
        {
            GameObject result = FindDeepChild(child.gameObject, targetName);
            if (result != null)
                return result;
        }
    ...
    private static void CreateAllHitBoxes()
    {
        ...
        List<string> groundCheckerObjects = new List<string>() { "GroundChecker_L", "GroundChecker_R" };
        ...
        foreach (var child in allChildren)
        {
            ...
            if (child.gameObject.name.Contains(_hitbox) || child.gameObject.name.Contains(_playerCapsule) || child.gameObject.name.Contains(_groundCheck))
            {
                ...
                foreach (var groundChecker in groundCheckerObjects)
                {
                    if (child.gameObject.name.Contains(groundChecker))
                    {
                        CreateCollider(child.gameObject, ColliderType.Sphere);
                        child.tag = "GroundChecker";
                    }
                }
    ...
    private static void EndSetting(GameObject obj, GameObject playerCapsule, GameObject groundCheckerL, GameObject groundCheckerR)
    {
        ...
        obj.GetComponent<CheckGround>().groundCheckPointR = groundCheckerR.transform;
        
        playerCapsule.GetComponent<CapsuleCollider>().isTrigger = true;
        playerCapsule.AddComponent<Rigidbody>();
        playerCapsule.GetComponent<Rigidbody>().automaticCenterOfMass = true;
        playerCapsule.GetComponent<Rigidbody>().automaticInertiaTensor = true;
        playerCapsule.GetComponent<Rigidbody>().isKinematic = false;
        playerCapsule.GetComponent<Rigidbody>().useGravity = true;
        playerCapsule.GetComponent<Rigidbody>().interpolation = RigidbodyInterpolation.Interpolate;
        playerCapsule.GetComponent<Rigidbody>().collisionDetectionMode = CollisionDetectionMode.Discrete;
        playerCapsule.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezeRotation;
